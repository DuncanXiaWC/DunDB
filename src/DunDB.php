...
class DunDB
{
    ...
    /**------------------------数据库链接方法------------------**/
    public static function i() : DunDB
    {
        // 判断是否支持pdo
    ...
    public function connect(string $connect = 'default') : void
    {
        ...
        if (null === $this->conf) {
            $this->conf = Config::get('My_DB');
        }
        $conf = $this->conf[$connect];
        // 传入链接是否存在,存在不重复创建
        if (!isset($this->linkPool[$connect])) {

            $dsn = ":host={$conf['host']};port={$conf['port']};DunDBname={$conf['DunDBname']};charset={$conf['charset']}";
    ...
    /**------------------------------crud方法-------------------**/
    /**
     * DunDB::i()->table('users as u')->leftJoin('levels as l', 'u.level', '=', 'l.id')
     ...
     *
     * DunDB::i()->table('users')->insert([['name' => 'lisi'],['name' => 'zs']]);
     * DunDB::i()->table('users')->insert(['name' => 'lisi', 'age' => 24]);
     * DunDB::i()->table('users')->insertGetId(['name' => 'lisi', 'age' => 24]);
     *
     * DunDB::i()->table('users')->where('id', '=', 1)->update(['name' => 'zhangsan']);
     * DunDB::i()->table('users')->where('id', '=', 1)->decrement('scores');
     * DunDB::i()->table('users')->where('id', '=', 1)->decrement('scores', 1);
     * DunDB::i()->table('users')->where('id', '=', 1)->decrement(['scores', 1], ['rest',-2]);
     *
     * DunDB::i()->table('users')->where('id', '=', 1)->delete();
     */
    /**
     ...
     * @return DunDB
     */
    public function table(string $table) : DunDB
    {
     ...
     * @return DunDB
     */
    public function select($fields) : DunDB
    {
     ...
     * @return DunDB
     */
    public function selectRaw($fields) : DunDB
    {
        $fields = is_array($fields) ? $fields : func_get_args();
        $fieldSql = '';
        foreach ($fields as $val) {
...
//            if ($field !== '*' && strpbrk($field, '.`(') === false) $field = '`' . $field . '`';
     ...
     * @return DunDB
     */
    public function where($where) : DunDB
    {
     ...
     * @return DunDB
     */
    public function orWhere($where) : DunDB
    {
        if (empty($where)) return $this;
        $whereSql = '';
        $whereParams = [];
     ...
     * @return DunDB
     */
    public function whereNull(string $field) : DunDB
    {
     ...
     * @return DunDB
     */
    public function whereNotNull(string $field) : DunDB
    {
        if (strpbrk($field,'.`(') === false) $field = '`' . $field . '`';
     ...
     * @return DunDB
     */
    public function whereBetween(string $field, array $between) : DunDB
    {
        if (strpbrk($field, '.`(') === false) $field = '`' . $field . '`';
     ...
     * @return DunDB
     */
    public function whereNotBetween(string $field, array $between) : DunDB
    {
        if (strpbrk($field, '.`(') === false) $field = '`' . $field . '`';
     ...
     * @return DunDB
     */
    public function whereIn(string $field, array $in) : DunDB
    {
        if (strpbrk($field, '.`(') === false) $field = '`' . $field . '`';
     ...
     * @return DunDB
     */
    public function whereNotIn(string $field, array $in) : DunDB
    {
        if (strpbrk($field, '.`(') === false) $field = '`' . $field . '`';
        $place_holders = implode(',', array_fill(0, count($in), '?'));
     ...
     * @return DunDB
     */
    public function whereColumn($where) : DunDB
    {
        if (empty($where)) return $this;
        $whereSql = '';
        if (is_array($where)) { // 二维数组
     ...
     * @return DunDB
     */
    public function leftJoin() : DunDB
    {
        $argNum = func_num_args();
        $params = func_get_args();
        $joinSql = 'left join ';
     ...
     * @return DunDB
     */
    public function rightJoin() : DunDB
    {
        $argNum = func_num_args();
        $params = func_get_args();
        $joinSql = 'right join ';
     ...
     * @return DunDB
     */
    public function orderBy(string $field, string $order) : DunDB
    {
        if (strpbrk($field, '.`(') === false) $field = '`' . $field . '`';
     ...
     * @return DunDB
     */
    public function groupBy(string $field) : DunDB
    {
        if (strpbrk($field, '.`(') === false) $field = '`' . $field . '`';
     ...
     * @return DunDB
     */
    public function limit(int $limit, int $length = 0) : DunDB
    {
    ...
    /**----------------------------原生sql方法----------------**/
    /**
     * DunDB::i()->prepare(原生sql)->execute(参数)->rowCount();
     * DunDB::i()->prepare(原生sql)->execute(参数)->lastInsertId();
     * DunDB::i()->prepare('update `users` set `status`=? where `id`>?')->execute([1,6])->rowCount();
     * DunDB::i()->prepare('insert into `users` (name,age) values (?,?)')->execute(['lisi',25])->lastInsertId();
     * DunDB::i()->prepare('select * from `users` where `id`>?')->execute([6])->fetchAll();
     * DunDB::i()->prepare('select * from `users` where `id`>?')->execute([6])->fetch();
     * @param string $sql
     * @return DunDB
     */
    public function prepare(string $sql) : DunDB
    {
        $this->pdoStatement = $this->link->prepare($sql);
        return $this;
    }

    public function execute(array $params = []) : DunDB
    {
    ...
    /**
     * DunDB::i()->exec('insert into `users` (name,age) values ('lisi',25)');
     * @param string $sql
     * @return int
     */
    ...
    /**
     * DunDB::i()->query('select * from `users` where `id`>6')->fetchAll();
     * @param string $sql
     * @return DunDB
     * @throws \Exception
     */
    public function query(string $sql) : DunDB
    {
    ...
    private function orGroupBegin() : DunDB
    {
        if ($this->sqlTemp['where'] === '') {
    ...
    private function andGroupBegin() : DunDB
    {
        if ($this->sqlTemp['where'] === '') {
            $this->sqlTemp['where'] = 'where (';
        } else {
